...
- The setting audit records the source and normalized indoor/outdoor distributions for all 463 activities. The source repeats `Indoor=false, Outdoor=true` across 395 rows, including complete indoor-oriented sections. For that repeated combination only, the importer retains classifications supported by explicit title, summary, tag, collection or weather evidence; `Home` and `Kitchen` weather values count as indoor evidence. Unsupported rows map to the honest neutral state (`false`/`false`) and their IDs are catalogued for manual metadata review. A matching data migration keeps already imported rows aligned with future imports.
- The environment/content consistency audit conservatively compares normalized setting metadata with explicit location language in each activity's title, summary, instructions, weather, tags and collections. Definite one-sided contradictions and broader manual-review cases are written to `environmentContentConsistencyAudit` in the validation report; the audit flags content for editorial review and never rewrites suitability automatically.
- The activity CSV's explicit `Resource IDs`/`Resource Use` fields are the source of truth for whether an activity declares a printable. A blank pair means no printable is required; a valid declared relationship with its verified manifest/PDF is available; malformed, missing or broken declared links remain a detectable integrity state. Counts and affected IDs are written to `activityResourceStateAudit` in the validation report.
