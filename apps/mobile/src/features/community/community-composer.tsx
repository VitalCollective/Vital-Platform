...
export function CommunityComposer({ api, initialType, onClose, onCreated }: {
  ...
  return <CommunityModal title="Share with Community" busy={busy} onClose={() => {
    if (title || body) setConfirmDiscard(true); else onClose();
  }}>
    {confirmDiscard ? <View style={s.card}><Text style={s.body}>Discard this draft?</Text><Text style={s.meta}>Your unpublished words will be cleared.</Text><Button label="Keep writing" onPress={() => setConfirmDiscard(false)} /><Button label="Discard draft" variant="danger" onPress={onClose} /></View> : null}
    <Text style={s.meta}>Share what worked, ask when you are stuck, or tell us how it really went.</Text>
    <View style={s.row}>{POST_TYPES.map((type) => <FilterChip key={type.value} label={type.noun} selected={postType === type.value} onPress={() => setPostType(type.value)} />)}</View>
    ...
    <View style={s.row}><FilterChip label="General" selected={!topic} onPress={() => setTopic(null)} />{COMMUNITY_TOPICS.map((value) => <FilterChip key={value} label={value} selected={topic === value} onPress={() => setTopic(value)} />)}</View>
    <Text style={s.label}>Link a Vital activity · optional</Text>
    {activity ? <View style={s.card}><Text style={s.body}>{activity.title}</Text><CommunityAction label="Remove activity link" icon="close" onPress={() => { setActivity(null); setActivitySearch(''); }} /></View> : <>
      ...
      {searching ? <Text style={s.meta}>Finding activities…</Text> : null}
      {!searching && query.trim().length >= 2 && !options.length && !activityError ? <Text style={s.meta}>No matching activities. Try a different part of the title.</Text> : null}
    ...
    <Text style={s.meta}>No advertising or sales pitches. Please keep children's identifying details private.</Text>
    <CommunityNotice message={error} error />
    <Button label="Publish post" icon="paper-plane-outline" onPress={() => void publish()} loading={busy} />
  </CommunityModal>;
}
