...
export function SavedActivitiesScreen({ profileId }: { profileId: string }) {
  ...
  const controller = useMemo(() => createSavedActivitiesController(createActivitySavesApi(requireSupabase(), language), profileId), [language, profileId]);
  ...
  return (
    <Screen scrollProps={{ refreshControl: <RefreshControl refreshing={state.isLoading && state.activities !== null} onRefresh={refresh} tintColor={colors.brand} colors={[colors.brand]} /> }}>
      <ScreenHeader title={t('Saved')} description={t('Good ideas to come back to, whenever you need them.')} />
      {state.activities !== null ? (
        <View style={styles.toolbar}>
          <Text style={styles.caption} accessibilityLiveRegion="polite">
            {t(activities.length === 1 ? '{count} saved activity · newest first' : '{count} saved activities · newest first', { count: activities.length })}
          </Text>
          <Button label={t('Refresh')} icon="refresh-outline" variant="secondary" loading={state.isLoading} onPress={refresh} />
        </View>
      ) : null}
      {state.error ? (
        <StatePanel kind="error" title={t('Your saved ideas are unavailable')} message={state.error} onRetry={refresh} />
      ) : state.isLoading && state.activities === null ? (
        <StatePanel kind="loading" title={t('Gathering your saved ideas')} message={t('Bringing your saved activities together.')} />
      ) : activities.length ? (
        <>
          ...
          {activities.length > state.visibleCount ? (
            <View style={styles.more}><Button label={t('Show more saved activities')} variant="secondary" disabled={state.isLoading} onPress={controller.showMore} /></View>
          ) : null}
        </>
      ) : (
        <View style={styles.empty}>
          <Ionicons name="bookmark-outline" size={30} color={colors.plum} accessible={false} />
          <Text style={styles.emptyTitle}>{t('Keep a few good ideas close')}</Text>
          <Text style={styles.emptyCopy}>{t('When an activity catches your eye, tap Save. You’ll find it here, ready for another day.')}</Text>
          <Button label={t('Find an activity')} icon="search-outline" onPress={() => router.navigate('/discover')} />
        </View>
      )}
    </Screen>
  );
