...
export function FeedbackSubmissionForm({ api, id, initialType = 'comment' }: { api: AccountApi; id: string; initialType?: FeedbackType }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice(t('Thank you. Your feedback has been sent privately to the Vital team.'));
    } catch (cause) {
  ...
  return <Group title={t('Send feedback')}>
    <Text style={a.body}>{t('Tell us about a bug, suggestion, comment, or anything else that would make Vital better for your family.')}</Text>
    <Text style={s.label}>{t('Feedback type')}</Text>
    <View style={s.row}>{FEEDBACK_TYPES.map(([value, label]) => <FilterChip key={value} label={t(label)} selected={type === value} onPress={() => { setType(value); setError(null); setNotice(null); }} />)}</View>
    ...
    <Text style={a.meta}>{t('Your submission is private and is not posted in Community.')}</Text>
    <CommunityNotice message={error} error /><CommunityNotice message={notice} />
    <Button label={t('Submit feedback')} icon="paper-plane-outline" loading={busy} onPress={() => void submit()} />
  </Group>;
}
...
export function ActivitySubmissionForm({ api, id }: { api: AccountApi; id: string }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setNotice(t('Thank you. Your activity has been sent privately to the Vital team for review.'));
    } catch (cause) {
  ...
  return <Group title={t('Suggest an activity')}>
    <Text style={a.body}>{t('Got an activity your family loves? Send it to us. If we add it to Vital, we’ll give you one month of Vital membership free.')}</Text>
    <CommunityField label="Activity name" value={name} onChangeText={(value) => { setName(value); changed(); }} maxLength={160} editable={!busy} />
    <Text style={s.label}>{t('Vital section · optional')}</Text>
    <View style={s.row}><FilterChip label={t('Not sure')} selected={!section} onPress={() => { setSection(null); changed(); }} />
      {SUBMISSION_SECTIONS.map(value => <FilterChip key={value} label={t(value)} selected={section === value} onPress={() => { setSection(value); changed(); }} />)}
    </View>
    ...
    <Text style={a.meta}>{t('If we accept it, Vital may edit, adapt and publish it clearly for other families. Duplicate or already-known activities may not qualify.')}</Text>
    <Text style={a.meta}>{t('The reward applies only when Vital accepts and publishes the activity, has no cash alternative, and will be fulfilled using the membership options supported by your subscription platform.')}</Text>
    <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: rightsConfirmed }} disabled={busy}
      ...
      <Text style={[a.body, a.grow]}>{t('I confirm this activity is my own, or I have the right to submit it to Vital.')}</Text>
    </Pressable>
    <Text style={a.meta}>{t('Your submission is private and is not posted in Community.')}</Text>
    <CommunityNotice message={error} error /><CommunityNotice message={notice} />
    <Button label={t('Submit activity')} icon="paper-plane-outline" loading={busy} onPress={() => void submit()} />
  </Group>;
}
