...
function EditProfile({ id, profile, onSaved, onPhotoChanged }: {
  ...
  return <Group title={t('How members see you')}>
    <Text style={a.meta}>{t('Your name, photo and introduction are visible in Community. Your email and family details are not part of your Community profile.')}</Text>
...
function MembershipPanel() {
  ...
  return <View style={a.stack}><Group title={t(membershipHeading(billing.membership))}>
    {billing.isResolving ? <Text style={a.body}>{t('Checking your membership…')}</Text> : <>
      ...
      {billing.state === 'active' && <Text style={a.body}>{endDate ? t('Your full Vital membership is active until its next renewal on {date}.', { date: endDate }) : t('Your full Vital membership is active.')}</Text>}
      {billing.state === 'cancelled' && <Text style={a.body}>{endDate ? t('Automatic renewal is off. You still have full Vital access until {date}.', { date: endDate }) : t('Automatic renewal is off. You still have full Vital access until the verified period ends.')}</Text>}
      {billing.state === 'grace_period' && <Text style={a.body}>{endDate ? t('Your store reported a payment problem, but your Vital access continues during the grace period until {date}.', { date: endDate }) : t('Your store reported a payment problem, but your Vital access continues during the current grace period.')}</Text>}
      {billing.state === 'billing_issue' && <Text style={a.body}>{t('Your store says the payment issue is no longer within a valid grace period. Update your payment method to restore access.')}</Text>}
      {['expired', 'refunded', 'revoked'].includes(billing.state) && <Text style={a.body}>{t('Choose a membership or restore an eligible store purchase to return to Vital.')}</Text>}
      {['no_entitlement', 'trial_available'].includes(billing.state) && <Text style={a.body}>{t('Choose a Vital membership to access activities, resources, Saved and Community.')}</Text>}
      {billing.state === 'restoring' && <Text accessibilityLiveRegion="polite" style={a.body}>{t('Checking your store purchases…')}</Text>}
      {billing.state === 'provider_unavailable' && <Text style={a.body}>{t('We couldn’t confirm your membership just now. Your account, support and legal options remain available.')}</Text>}
      {billing.error && <CommunityNotice message={billing.error} error />}
      {billing.error && <Button label={t('Try again')} variant="secondary" onPress={() => void billing.refresh()} />}
    </>}
  </Group>
    <Group title={t('Vital membership plans')}>
      {displayedPlans.map(plan => <View key={plan.id} style={a.rule}>
        <Text style={a.label}>{t(plan.title)}</Text><Text style={a.title}>{plan.price} <Text style={a.meta}>{t(plan.interval)}</Text></Text>
        {'trialDescription' in plan && plan.trialDescription && <Text style={a.meta}>{t(plan.trialDescription)}</Text>}
        {'packageIdentifier' in plan && !billing.hasAccess && <Button
          label={plan.trialDescription
            ? t('Start {trial} · {price}', { trial: plan.trialDescription, price: plan.price })
            : t('Choose {plan} · {price}', { plan: t(plan.title).toLocaleLowerCase(), price: plan.price })}
      ...
      {displayedPlans.length > 0 && <Text style={a.body}>{t('The standard introductory trial is 7 days and converts to paid membership unless cancelled through Apple or Google before it ends.')}</Text>}
      {showDevelopmentFallbackPlans && <Text style={a.meta}>{t('Development-only fallback prices are shown for testing. No payment or trial can start until store products load.')}</Text>}
      {billing.plans.length === 0 && !showDevelopmentFallbackPlans && <Text style={a.meta}>{t('Membership plans are temporarily unavailable. Please try again.')}</Text>}
      {billing.plans.length === 0 && !billing.error && <Button label={t('Try again')} variant="secondary" onPress={() => void billing.refresh()} />}
      {billing.plans.length > 0 && !billing.purchasesEnabled && <Text style={a.meta}>{t('Membership purchases are unavailable in this version of Vital.')}</Text>}
      <Button label={t('Restore purchases')} variant="secondary" disabled={!billing.providerAvailable || Boolean(billing.busyAction)}
        loading={billing.busyAction === 'restore'} onPress={() => void billing.restore()} />
      {billing.managementUrl && <Button label={t('Manage subscription')} variant="secondary"
...
function CommunityPanel({ community, navigate, openCommunity }: Pick<Props, 'community' | 'navigate' | 'openCommunity'>) {
  ...
  return <View style={a.stack}>
    ...
    <Group title={t('Reporting & moderation')}><Text style={a.body}>{t('Use Report on a post or reply to raise a concern with the Vital team. Reports are not shown publicly.')}</Text>
      <Text style={a.meta}>{t('You can also block a member from the report options. Your posts and replies will be hidden from each other. Community rules explain what is expected when taking part.')}</Text></Group>
...
function PrivacySafetyPanel({ navigate }: Pick<Props, 'navigate'>) {
  ...
  return <View style={a.stack}>
    ...
    <Group title={t('Your privacy')}><Text style={a.body}>{t('Your family information stays private to your account. Your Community profile contains only the name, image and introduction you choose to share.')}</Text>
...
function DeleteAccountPanel({ api, id, navigate, signOut }: Pick<Props, 'api' | 'id' | 'navigate' | 'signOut'>) {
  ...
  return <View style={a.stack}>
    <Group title="Delete your account">
      <Text style={a.body}>{t('This permanently removes your Vital account, profile, family information, preferences, Saved items and Community content. It cannot be undone.')}</Text>
      <Text style={a.meta}>{t('If another member has replied to one of your Community posts, only a neutral deleted-post marker may remain so their reply is not destroyed. Your original text, name, introduction and profile image will be removed.')}</Text>
      <Text style={a.meta}>{t('Deleting your Vital account does not cancel your App Store or Google Play subscription.')}</Text>
      ...
      {!confirming ? <>
        ...
        <Text style={a.body}>{t('Type DELETE below to confirm permanent account deletion.')}</Text>
        <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: billingAcknowledged }}
          ...
          <Text style={a.meta}>{t('I understand that deleting Vital does not cancel billing through Apple or Google.')}</Text>
        </Pressable>
...
function InformationPanel({ panel, navigate }: { panel: AccountPanel; navigate: Props['navigate'] }) {
  const { t } = useLanguage();
  if (panel === 'about') return <Group title="Vital Collective"><Text style={a.body}>{t('Activities, ideas and resources for you and your family, across Vital Mums, Vital Kids, Vital Together, Vital Life and Vital Food.')}</Text>
    <Text style={a.meta}>{t('Discover something to try, save it for another day, and share experiences in Community.')}</Text><Text style={a.meta}>{t('App version {version}', { version: Constants.expoConfig?.version ?? t('unavailable') })}</Text></Group>;
  ...
  return <Group title={ACCOUNT_PANELS[panel]}>
    <Text style={a.body}>{t(panel === 'problem' ? 'Tell us what you were doing, what happened and which device you were using. Please do not include passwords or private family information.' : 'Questions about using Vital or your account belong here.')}</Text>
...
export function AccountScreen({ id, email, panel, api, community, navigate, openCommunity, signOut }: Props) {
  ...
  return <Screen key={panel ?? 'hub'} keyboardAware={panel === 'profile' || panel === 'family' || panel === 'suggest' || panel === 'feedback' || panel === 'problem' || panel === 'deletion'} scrollProps={{ keyboardDismissMode: 'on-drag' }}>
    ...
    <View style={a.stack}><CommunityNotice message={notice} />
      {!panel ? <>
        <Group title={t('Profile')}><AccountLoadState state={profile} />{!profile.loading && !profile.error && identity}{link('profile')}{link('identity')}</Group>
        <View style={a.grid}><View style={[a.column, isTablet && a.wideColumn]}>
          <Group title={t('Your family')}>{link('family', 'Family members & ages', 'Keep your private family details in one place.')}</Group>
          <Group title={t('Preferences')}>{link('preferences', 'Activities, notifications & Vital news')}</Group>
          <Group title={t('Vital membership')}>{link('membership', 'Manage Vital membership')}</Group>
          <Group title={t('Community')}>{link('identity')}{link('community', 'Rules, starter conversations & reporting')}</Group>
          <Group title={t('Privacy & safety')}>{link('privacySafety', 'Privacy & safety', 'Manage Community blocks and review how your information is used.')}</Group>
        </View><View style={[a.column, isTablet && a.wideColumn]}>
          <Group title={t('Help & feedback')}>{link('suggest', 'Suggest an activity', 'Share something your family genuinely enjoys.')}{link('feedback', 'Send feedback', 'Comments, suggestions or product feedback.')}{link('help')}{link('problem')}</Group>
          <Group title={t('About & legal')}>{link('about')}{link('privacy')}{link('terms')}<Text style={a.meta}>{t('Version')} {Constants.expoConfig?.version ?? t('unavailable')}</Text></Group>
          <Group title={t('Account')}><CommunityNotice message={error} error /><Button label={t('Sign out')} icon="log-out-outline" variant="secondary" loading={busy} onPress={() => void leave()} />{link('deletion')}</Group>
        </View></View>
        ...
        <AccountLoadState state={profile} />{!profile.loading && !profile.error && (profile.value ? panel === 'profile' ?
          <EditProfile id={id} profile={profile.value}
            onSaved={saved => { profile.commit(saved); navigate(null); setNotice(t('Your profile has been saved.')); }}
            onPhotoChanged={profile.commit} /> :
          <Group title={t('Your Community identity')}>{identity}<Text style={a.meta}>{t('This is the name, image and introduction members see beside your conversations.')}</Text>{link('profile')}<AccountLink label="Open Community" onPress={openCommunity} /></Group> : <><Text style={a.body}>{t('Your profile is unavailable.')}</Text><Button label="Try again" onPress={profile.reload} /></>)}
        ...
        panel === 'privacySafety' ? <PrivacySafetyPanel navigate={navigate} /> : panel === 'blocked' ? <Group title={t('Blocked members')}><BlockedMembersContent api={community} onChanged={() => setNotice(t('This member is no longer blocked.'))} /></Group> :
...
export function MembershipAccessScreen({ id, email, panel, api, navigate, signOut }: Pick<Props, 'id' | 'email' | 'panel' | 'api' | 'navigate' | 'signOut'>) {
  ...
  const link = (next: AccountPanel, label: string = ACCOUNT_PANELS[next]) =>
    <AccountLink label={t(label)} onPress={() => navigate(next)} />;
  return <Screen key={destination} keyboardAware={destination === 'suggest' || destination === 'feedback' || destination === 'problem' || destination === 'deletion'}>
    {destination !== 'membership' && <AccountLink direction="back" label={t('Back to membership')} onPress={() => navigate('membership')} />}
    <ScreenHeader eyebrow={t('Your Vital')} title={t(ACCOUNT_PANELS[destination])}
      description={destination === 'membership' ? t('Signed in as {member}', { member: email ?? t('a Vital member') }) : undefined} />
    <View style={a.stack}>
